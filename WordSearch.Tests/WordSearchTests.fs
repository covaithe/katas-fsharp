module Tests
    ...
    [<Fact>]
    let ``parse should extract cells describing the puzzle layout``() =
        let input = dedent """
            a,b
            c,d
        """
        ...
        Assert.Equal<Cell seq>(expectedCells, parse input)

    [<Fact>]
    let ``find should return the location of the cells making up the word``() =
    ...
    [<Fact>]
    let ``find should locate words starting from any cell matching the word start``() =
